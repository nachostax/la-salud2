...
function setProgressPeriod(period) {
  ...
  ['week','month','year'].forEach(p => {
    const el = document.getElementById('prog-per-' + p);
    if (el) el.classList.toggle('active', p === period);
  });
  renderTrends();
}

// Weight / Food / Activity sub-tabs, shown above the charts. Defaults to
// 'weight'. Stored on S so it persists across re-renders within a session.
...
function renderTrends() {
  ...
  const rangeDays = progressPeriod === 'week' ? 7 : progressPeriod === 'year' ? 365 : 30;
  const dates = [];
  for (let i=rangeDays-1;i>=0;i--) {
    const d = new Date(); d.setDate(d.getDate()-i);
    dates.push(toLocalDateStr(d));
  }
  const rangeLabel = progressPeriod === 'week' ? 'last 7 days' : progressPeriod === 'year' ? 'last 365 days' : 'last 30 days';
  ...
  // ── Weight / Food / Activity submenu ──
  // Sits above the Week/Month/Year chart toggle. Filters which trend cards
  // render below it. Defaults to 'weight' on first render.
  const progressCategory = S.progressCategory || 'weight';
  const catHtml = `<div class="seg" id="prog-cat-toggle" style="margin-bottom:16px">
    <div class="seg-opt${progressCategory==='weight'?' active':''}" id="prog-cat-weight" onclick="setProgressCategory('weight')">Weight</div>
    <div class="seg-opt${progressCategory==='food'?' active':''}" id="prog-cat-food" onclick="setProgressCategory('food')">Food</div>
    <div class="seg-opt${progressCategory==='activity'?' active':''}" id="prog-cat-activity" onclick="setProgressCategory('activity')">Activity</div>
  </div>`;
  ...
  el.innerHTML = catHtml + (html || '<div class="empty-state">Not enough data yet.<br>Keep logging meals and weight.</div>');
}
