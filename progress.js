...
// Positions/sizes the shared sliding bar under whichever period option is
// active, mirroring how .bnav-indicator tracks the active bottom-nav tab.
// Needs rAF on first render because the bar has just been written into the
// DOM via innerHTML and offsetLeft/offsetWidth aren't reliable until layout
// has happened.
function positionProgressPeriodIndicator() {
  const bar = document.getElementById('prog-period-bar');
...
window.addEventListener('resize', positionProgressPeriodIndicator);
...
function renderTrends() {
  ...
  const periodBarHtml = `<div class="prog-period-bar" id="prog-period-bar">
    ...
    <div class="prog-period-indicator" id="prog-period-indicator"></div>
  </div>`;
  ...
  if (progressCategory === 'food') {
    ...
    const avg = completeDays.length ? Math.round(completeDays.reduce((a,b)=>a+b.total,0)/completeDays.length) : 0;
    const points = completeDays.map(d=>({x:d.date, y:d.total}));
    const svg = makeSVGLine(points, carbsColor, 320, 80, target);
    const labels = completeDays.length >= 2
      ? [completeDays[0].date.slice(5), completeDays[completeDays.length-1].date.slice(5)]
      : ['—','—'];
  ...
  if (progressCategory === 'food') {
    ...
    const avg = completeDays.length ? Math.round(completeDays.reduce((a,b)=>a+b.total,0)/completeDays.length) : 0;
    const points = completeDays.map(d=>({x:d.date, y:d.total}));
    const svg = makeSVGLine(points, fatColor, 320, 80, target);
    const labels = completeDays.length >= 2
      ? [completeDays[0].date.slice(5), completeDays[completeDays.length-1].date.slice(5)]
      : ['—','—'];
  ...
  // Indicator needs an actual layout pass on the freshly-written bar before
  // offsetLeft/offsetWidth are meaningful — rAF covers renderTrends() being
  // called from anywhere (tab open, person toggle, category switch, period
  // switch), not just setProgressPeriod.
  requestAnimationFrame(positionProgressPeriodIndicator);
}
