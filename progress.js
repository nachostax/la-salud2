...
if (!window._mcDotListenerBound) {
  ...
  document.addEventListener('click', e => {
    ...
    showToast(`${_fmtChartDate(dot.dataset.date)} — ${display}${dot.dataset.unit ? ' '+dot.dataset.unit : ''}`);
  });
}
...
function makeSVGLine(points, color, width, height, target, yAxisMin, unitMode, fixedRange, valueLabel) {
  ...
  const svg = `<svg class="mini-chart-svg" viewBox="0 0 ${width} ${height}" preserveAspectRatio="none" data-h="${height}">
    ...
    ${coords.filter(c=>c).map(c => `<circle class="mc-dot" data-date="${c.date}" data-val="${c.val}" data-unit="${valueLabel||''}" cx="${c.x.toFixed(1)}" cy="${c.y.toFixed(1)}" r="1.3" fill="${color}" style="cursor:pointer"/>`).join('')}
  </svg>`;
...
function setProgressPeriod(period) {
  ...
  newCharts.forEach((chart, i) => {
    ...
    const dotInfo = Array.from(dots).map(dot => {
      ...
      if (prev) {
        dot.style.opacity = '1';
        return { dot, newCx, newCy, dx: prev.cx-newCx, dy: prev.cy-newCy, entering:false };
      } else {
        dot.style.opacity = '0';
        return { dot, newCx, newCy, dx:0, dy:0, entering:true };
      }
    });
    ...
    function step(now) {
      ...
      // Dots and area both read off the SAME t/e every frame — they can't drift.
      dotInfo.forEach(d => {
        if (d.entering) { d.dot.style.opacity = String(e); }
        else { d.dot.style.transform = `translate(${(d.dx*(1-e)).toFixed(1)}px,${(d.dy*(1-e)).toFixed(1)}px)`; }
      });
      if (canMorph) {
...
function renderTrends() {
  ...
  if (progressCategory === 'food') {
    ...
    const { svg, labelsHtml } = makeSVGLine(points, color, 320, 80, target, null, null, range, 'kcal');
  ...
  if (progressCategory === 'food') {
    ...
    const { svg, labelsHtml } = makeSVGLine(points, proteinColor, 320, 80, target, null, null, range, 'g');
    const labels = [dates[0].slice(5), dates[dates.length-1].slice(5)];
  ...
  if (progressCategory === 'food') {
    ...
    const { svg, labelsHtml } = makeSVGLine(points, carbsColor, 320, 80, target, null, null, range, 'g');
    const labels = [dates[0].slice(5), dates[dates.length-1].slice(5)];
  ...
  if (progressCategory === 'food') {
    ...
    const { svg, labelsHtml } = makeSVGLine(points, fatColor, 320, 80, target, null, null, range, 'g');
    const labels = [dates[0].slice(5), dates[dates.length-1].slice(5)];
