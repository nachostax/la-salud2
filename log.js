// ── LOG TAB OPEN/CLOSE ANIMATION ──────────────────────────────────────────
// ui.js owns the generic left/right slide transition for every .sec via
// showSec(name, el). The Log tab gets a special-case animation instead: the
// "+" circle in the bottom nav expands to "swallow" the screen while the
// log panel itself slides up from the bottom and fades in; reverse on the
// way out. Rather than editing ui.js's showSec (out of scope here), this
// wraps the global showSec once log.js loads, running the extra animation
// around whatever ui.js already does. Safe to call multiple times — only
// wraps once, guarded by window.__logTabAnimWrapped.
(function setupLogTabAnimation() {
  if (window.__logTabAnimWrapped) return;
  if (typeof window.showSec !== 'function') return; // ui.js not loaded yet — bail quietly, nothing to wrap
  window.__logTabAnimWrapped = true;

  const originalShowSec = window.showSec;
  let lastSec = null;

  window.showSec = function(name, el) {
    const enteringLog = name === 'log' && lastSec !== 'log';
    const leavingLog = name !== 'log' && lastSec === 'log';

    if (enteringLog) {
      const circle = document.querySelector('.bnav-log-icon circle');
      if (circle) {
        circle.classList.remove('bnav-log-expanding');
        // restart the animation even if it's mid-flight from a rapid re-tap
        void circle.getBoundingClientRect();
        circle.classList.add('bnav-log-expanding');
        setTimeout(() => circle.classList.remove('bnav-log-expanding'), 420);
      }
    }
    ...
    const result = originalShowSec.apply(this, arguments);
...
// ── LOG WATER FOR A DIFFERENT DATE ────────────────────────────────────────
// The retro-date toggle/clear functions live in ui.js and only know about
// meal/workout retro-logging. Wrapped here (same pattern as showSec above)
// so opening/closing the retro-date row also shows/hides and pre-fills the
// water block underneath, without ui.js needing to know it exists.
(function setupRetroWaterSync() {
  if (window.__retroWaterWrapped) return;
  if (typeof window.toggleRetroDate !== 'function' || typeof window.clearRetroDate !== 'function') return;
  window.__retroWaterWrapped = true;

  const originalToggle = window.toggleRetroDate;
  const originalClear = window.clearRetroDate;

  window.toggleRetroDate = function() {
    const result = originalToggle.apply(this, arguments);
    syncRetroWaterBlock();
    return result;
  };

  window.clearRetroDate = function() {
...
// Shows/hides the retro-water block to match the retro-date row, and keeps
// its date input following whatever date is selected up there — the water
// entry should land on the same date as the retro meal/time being logged.
function syncRetroWaterBlock() {
  const row = document.getElementById('retro-date-row');
  const block = document.getElementById('retro-water-block');
  if (!row || !block) return;
  const open = row.style.display !== 'none';
  block.style.display = open ? 'block' : 'none';
  if (open) {
    const dateInput = document.getElementById('retro-date-input');
    const waterDateInput = document.getElementById('retro-water-date-input');
    if (dateInput && waterDateInput && dateInput.value) waterDateInput.value = dateInput.value;
  }
}

// Keeps the retro-water date glued to the main retro date as the person
// edits it (not just at the moment the row was opened). Attached once.
(function wireRetroDateInputSync() {
  if (window.__retroWaterDateInputWired) return;
  const dateInput = document.getElementById('retro-date-input');
  if (!dateInput) return; // index.html not parsed yet — log.js loads after the markup, so this should always be present
  window.__retroWaterDateInputWired = true;
  dateInput.addEventListener('change', () => {
    const waterDateInput = document.getElementById('retro-water-date-input');
    if (waterDateInput && dateInput.value) waterDateInput.value = dateInput.value;
  });
})();

// Adds ml to whatever date is set in the retro-water date input (falls back
// to today if left blank). Writes straight to that date's water entry —
// mirrors addWaterMl()/setWaterMl() but targets an arbitrary date instead
// of always today.
function getRetroWaterDate() {
  const input = document.getElementById('retro-water-date-input');
  return (input && input.value) ? input.value : todayStr();
}

function addRetroWaterMl(delta) {
  const date = getRetroWaterDate();
  const person = S.currentPerson;
  const current = getWaterMlForEntry(getWaterEntry(person, date));
  setWaterMlForDate(person, date, current + delta);
  showToast('Added ' + delta + ' ml for ' + date);
}

// Reads the typed amount field and sets (not adds) that date's water total —
// for correcting/backfilling a specific day's total rather than tapping +250
// repeatedly.
function submitRetroWaterAmount() {
  const amountInput = document.getElementById('retro-water-amount-input');
  const ml = parseInt(amountInput && amountInput.value, 10);
  if (!ml || ml < 0) { showToast('Enter an amount in ml first'); return; }
  const date = getRetroWaterDate();
  setWaterMlForDate(S.currentPerson, date, ml);
  if (amountInput) amountInput.value = '';
  showToast('Set water to ' + ml + ' ml for ' + date);
}
...
// main Water tab keeps behaving exactly as before.
...
// ── WATER TAB: LOG FOR A DIFFERENT DATE ───────────────────────────────────
// Standalone retro-date entry point living directly in the Water panel —
// distinct from the meal panel's retro-water block (which only appears once
// a retro meal/time row is open). This lets water be backfilled into history
// on its own, with no meal logging involved. Same write path
// (setWaterMlForDate) as everywhere else; just its own input IDs so the two
// retro-water UIs never collide if both happen to be open at once.
function toggleWaterRetroDate() {
  const row = document.getElementById('wtr-retro-row');
  const actions = document.getElementById('wtr-retro-actions');
  if (!row) return;
  const opening = row.style.display === 'none';
  row.style.display = opening ? 'flex' : 'none';
  if (actions) actions.style.display = opening ? 'flex' : 'none';
  if (opening) {
...
function clearWaterRetroDate() {
  ...
  const actions = document.getElementById('wtr-retro-actions');
  const dateInput = document.getElementById('wtr-retro-date-input');
  const amountInput = document.getElementById('wtr-retro-amount-input');
...
function addWaterRetroMl(delta) {
...
function markDayComplete() {
  ...
  if (!today.length) {
    showToast('Log at least one meal today before marking the day complete');
    return;
  }
...
function syncMarkCompleteBtn() {
  ...
  if (label) label.textContent = isFull ? 'Day complete — tap to undo' : 'Mark day as complete';
...
function renderLogTab() {
  ...
  if (!cloudReady) {
    ...
    if (el) el.innerHTML = '<div class="empty-state" style="color:var(--mist);font-size:12px;font-family:\'JetBrains Mono\',monospace;letter-spacing:1px">⟳&nbsp;Syncing…</div>';
    return;
  }
  const el = document.getElementById('today-entries');
  ...
  const celebrationHtml = showCongrats ? `<div id="congrats-banner" style="text-align:center;padding:20px 10px 16px;margin-bottom:10px;border-bottom:1px solid var(--clay)">
      ...
      <div style="font-family:'Playfair Display',serif;font-size:30px;font-style:italic;color:${personC};letter-spacing:0.5px;line-height:1.1">Congratulations</div>
      <div style="font-family:'JetBrains Mono',monospace;font-size:10px;letter-spacing:3px;color:var(--mist);text-transform:uppercase;margin-top:8px;opacity:0.7">Day complete ✦</div>
    </div>` : '';

  if (!all.length) {
    el.innerHTML = celebrationHtml + '<div class="empty-state">Nothing logged for ' + (S.currentPerson==='gabi'?'Gabi':'Nacho') + ' today yet.</div>';
    if (showCongrats) launchConfetti();
    return;
  }

  el.innerHTML = celebrationHtml + all.map(e => {
