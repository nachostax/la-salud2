...
function potatesScoreForPerson(person, dates) {
  ...
  // HIIT & Strength now read from the session-based settings (settings.js),
  // not the legacy minute-based movementTargets — that schema is stale.
  const mt = (S.settings && S.settings.movementTargets && S.settings.movementTargets[person]) || {};
  const ss = S.settings || {};
  const TARGET_ZONE2     = mt.zone2_min_week        || 150;
  const TARGET_HIIT_SESS = ss.hiitSessions?.[person]     ?? 1;
  const TARGET_HIIT_MINS = ss.hiitMins?.[person]         ?? 30;
  const TARGET_STR_SESS  = ss.strengthSessions?.[person] ?? 3;
...
function renderVitals() {
  ...
  if (workouts.length) {
    ...
    workouts.forEach(w => {
      ...
      else if (t === 'HIIT' || t === 'Cardio-HIIT' || t === 'VO2Max' || (t === 'Cardio' && w.intensity === 'HIIT')) { hiitMin += (w.duration_min||0); hiitCount++; }
    ...
    const pillars = [
      { label:'Walking',  count: walkCount,     valText: walkSteps ? walkSteps.toLocaleString()+' steps' : '' },
      { label:'Cardio',   count: cardioCount,   valText: cardioMin ? cardioCount+'× · '+cardioMin+'min' : '' },
      { label:'HIIT',     count: hiitCount,     valText: hiitMin ? hiitCount+'× · '+hiitMin+'min' : '' },
      { label:'Strength', count: strengthCount, valText: strengthMin ? strengthCount+'× · '+strengthMin+'min' : '' },
      { label:'Mobility', count: mobCount,      valText: mobMin ? mobCount+'× · '+mobMin+'min' : '' },
    ].filter(p => p.count > 0);
    ...
    const lines = pillars.map(p => `
      ...
      <div class="wk-bar-bg"><div class="wk-bar-fill" style="width:${(p.count/maxCount*100)}%"></div></div>
    `).join('');
  ...
  document.getElementById('vitals-body').innerHTML = '<div class="psh-date" id="hdr-date" style="margin-top:-6px;opacity:0.85"></div>' + renderPotatesHero() + heroHtml + renderMicronutrientsCard(person) + wkHtml + renderLowsPanel(person, todayStr());
...
function renderPotatesHero() {
  ...
  return `<div class="day-card potates-score-card" style="border-color:var(--ochre);margin-bottom:10px">
    <div class="day-card-body" style="text-align:center;padding:10px 0 8px">
      <div class="potates-score-label" style="font-family:'JetBrains Mono',monospace;font-size:10px;letter-spacing:2px;color:var(--ochre);text-transform:uppercase;margin-bottom:4px">Potates Score</div>
