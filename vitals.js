...
function potatesScoreForPerson(person, dates) {
  ...
  // Defaults: Steps 7000/day | Zone2 150min/wk | VO2Max 30min/wk | Strength 90min/wk | Mobility 2 sessions/wk × 15min
  // Per-person overrides are read from S.settings.movementTargets[person].
  const mt = (S.settings && S.settings.movementTargets && S.settings.movementTargets[person]) || {};
  const TARGET_ZONE2    = mt.zone2_min_week        || 150;
  const TARGET_VO2MAX   = mt.vo2max_min_week        || 30;
  const TARGET_STRENGTH = mt.strength_min_week      || 90;
  const TARGET_MOB_SESS = mt.mobility_sessions_week || 2;
  const TARGET_MOB_MIN  = mt.mobility_min_session   || 15;
  const TARGET_STEPS    = mt.steps_day              || 7000;
  ...
  dates.forEach(d => {
    dayCache.get(d).filter(e=>e.record_type==='workout'&&
      ...
      .forEach(w=>{ vo2maxMin+=(w.duration_min||0); });
  });
  const vo2maxPts = (vo2maxMin===0&&activeDays.size===0) ? null : Math.min(5,Math.round((vo2maxMin/Math.max(1,TARGET_VO2MAX*wf))*5));
  ...
  const strengthPts = (strengthMin===0&&activeDays.size===0) ? null : Math.min(5,Math.round((strengthMin/Math.max(1,TARGET_STRENGTH*wf))*5));
  ...
  return {
    ...
    moveData:{avgSteps:Math.round(avgSteps),zone2Min:Math.round(zone2Min),vo2maxMin:Math.round(vo2maxMin),strengthMin:Math.round(strengthMin),avgMobMin:Math.round(avgMobMin)},
...
function renderMicronutrientsCard(person) {
  ...
  return `<div class="trend-card">
    <div class="trend-card-title ${person==='gabi'?'tc-gabi':'tc-nacho'}">Micronutrients</div>
    ${microHtml}
  </div>`;
}
...
function renderVitals() {
  ...
  const heroLabel = S.period === 'day' ? 'kcal logged today' : 'avg kcal / day';
  ...
  if (isToday) {
    ...
    deltaBlock = `<div class="deficit-label" style="color:${deltaColor};font-size:26px">${avgDelta>=0?'+':''}${Math.round(avgDelta)} kcal</div>`;
  } else {
    ...
    <div class="donut-block">
      <div class="donut-row">
        ...
        <div class="donut-legend">
          ...
          <div class="legend-item"><div class="legend-dot" style="background:var(--terra)"></div><span class="legend-txt">Fat</span><span class="legend-val">${Math.round(fat)} / ${m.fat||'—'}g</span></div>
        </div>
      </div>
    </div>`;
  ...
  const heroHtml = `
    <div class="day-card">
      <div class="day-card-hdr">
        ${isToday && todayFull ? `<div class="day-card-check" title="Day complete">${checkmarkSvg}</div>` : ''}
      </div>
  ...
  if (workouts.length) {
    ...
    const TYPE_ORDER = ['Walking','Cardio','Cardio-Zone2','Zone 2','HIIT','Cardio-HIIT','VO2Max','Strength','Mobility','Stretching','Yoga'];
    const sortedEntries = Object.entries(byType).sort((a,b) => {
      const ai = TYPE_ORDER.findIndex(t => a[0].toLowerCase().includes(t.toLowerCase()));
      const bi = TYPE_ORDER.findIndex(t => b[0].toLowerCase().includes(t.toLowerCase()));
      return (ai === -1 ? 999 : ai) - (bi === -1 ? 999 : bi);
    });
    const lines = sortedEntries.map(([type,v]) => `
    ...
    wkHtml = `<div class="wk-block">
      <div class="meals-label" style="font-family:'JetBrains Mono',monospace;font-size:10px;letter-spacing:2px;color:var(--mist);text-transform:uppercase;margin-bottom:4px">Movement</div>
      <div class="wk-row">${lines}</div>
    </div>`;
  } else {
    wkHtml = `<div class="wk-block"><div class="meals-label" style="font-family:'JetBrains Mono',monospace;font-size:10px;letter-spacing:2px;color:var(--mist);text-transform:uppercase;margin-bottom:4px">Movement</div><div style="color:var(--mist);font-size:13px">No workouts logged in this period yet.</div></div>`;
  }

  document.getElementById('vitals-body').innerHTML = '<div class="psh-date" id="hdr-date"></div>' + renderPotatesHero() + heroHtml + renderMicronutrientsCard(person) + wkHtml + renderLowsPanel(person, todayStr());
  // hdr-date now lives inside vitals-body (above the score card) and is
  // recreated on every renderVitals() call, so it's populated directly
  // here rather than relying on a separate one-time setter elsewhere.
...
function renderPotatesHero() {
  ...
  const bdHtml =
    ...
    bdr('VO₂max / HIIT', bd.moveDetail.vo2maxPts, 5, bd.moveData.vo2maxMin > 0 ? bd.moveData.vo2maxMin + ' min this period' : 'no data') +
