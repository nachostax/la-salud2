...
async function _fetchFromServer() {
  ...
  try {
    ...
    // ── PERSON CASING REPAIR ──────────────────────────────────────────────
    // Some entries (originally from a CSV import that didn't normalise the
    // person column) ended up stored as "Nacho"/"NACHO" instead of the
    // lowercase 'nacho' every comparison in the app expects (entriesFor,
    // groupEntriesByPersonDate, the POTATES scorer, etc). A casing mismatch
    // there means those entries are silently invisible everywhere — which is
    // what was driving Nacho's score to a flat 0 despite real logged data.
    // Repaired on every load so it's fixed regardless of where the bad
    // casing originated, and self-heals if it ever creeps back in.
    let _personCasingFixed = false;
    [S.entries, S.weightLog].forEach(arr => {
      arr.forEach(e => {
        if (typeof e.person === 'string') {
          const fixed = e.person.trim().toLowerCase();
          if (fixed !== e.person) { e.person = fixed; _personCasingFixed = true; }
        }
      });
    });
    if (_personCasingFixed) {
      // Push the corrected casing back to the cloud so this is a one-time
