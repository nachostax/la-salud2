// Declared up-front (not down near renderHistory) so it's initialized before
// init() runs at the bottom of this file — init() -> setPerson() -> renderHistory()
// was hitting this in the temporal dead zone when it was declared later in the file.
...
function renderHistory() {
  ...
  el.innerHTML = allDates.map(date => {
    ...
    return `<div class="day-card hist-day-card" data-date="${date}">
...
// ── DATE JUMP — calendar icon in Settings ▸ History ─────────────────────────
// Opens the native date picker; jumpToHistoryDate() then does the actual
// scroll-and-expand once a date is chosen.
...
function jumpToHistoryDate(value) {
  if (!value) return;
  const cards = Array.from(document.querySelectorAll('#history-content .hist-day-card[data-date]'));
  if (!cards.length) return;
  // History renders newest-first. Prefer an exact match; otherwise the
  // closest day before the picked date (most recently logged before it),
  // falling back to the closest day after if nothing earlier exists.
  let target = cards.find(c => c.dataset.date === value)
    || cards.find(c => c.dataset.date < value)
    || cards[cards.length - 1];
  if (!target) return;
  const wrap = target.querySelector('.hday-detail-wrap');
  if (wrap && !wrap.classList.contains('open')) toggleHistoryDay(wrap.id);
  target.scrollIntoView({ behavior: 'smooth', block: 'center' });
  target.classList.add('hday-jump-flash');
  setTimeout(() => target.classList.remove('hday-jump-flash'), 1200);
}
