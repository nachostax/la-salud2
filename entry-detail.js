  ...
  inner.innerHTML = `
    <div style="padding:52px 20px 40px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
        <div class="trend-card"><div style="font-size:11px;color:var(--mist);letter-spacing:1px;text-transform:uppercase;margin-bottom:4px">Duration</div><div style="font-size:24px;font-weight:700;color:${personColor}">${e.duration_min || '—'}<span style="font-size:13px;font-weight:400;color:var(--mist)"> min</span></div></div>
        <div class="trend-card"><div style="font-size:11px;color:var(--mist);letter-spacing:1px;text-transform:uppercase;margin-bottom:4px">Burned</div><div style="font-size:24px;font-weight:700;color:${personColor}">${e.calories_burned ? Math.round(e.calories_burned) : '—'}<span style="font-size:13px;font-weight:400;color:var(--mist)"> kcal</span></div></div>
        <div class="trend-card"><div style="font-size:11px;color:var(--mist);letter-spacing:1px;text-transform:uppercase;margin-bottom:4px">Intensity</div><div style="font-size:18px;font-weight:600;color:var(--sand)">${e.intensity || '—'}</div></div>
        <div class="trend-card"><div style="font-size:11px;color:var(--mist);letter-spacing:1px;text-transform:uppercase;margin-bottom:4px">Type</div><div style="font-size:18px;font-weight:600;color:var(--sand)">${e.workout_type || '—'}</div></div>
      </div>
      ${e.notes ? `<div class="trend-card" style="margin-top:12px"><div style="font-size:11px;color:var(--mist);letter-spacing:1px;text-transform:uppercase;margin-bottom:4px">Notes</div><div style="font-size:14px;color:var(--sand)">${e.notes}</div></div>` : ''}
    </div>`;
}
    ...
    <div style="display:flex;align-items:center;gap:20px;padding:18px 16px 4px">
      <div style="position:relative;width:120px;height:120px;flex-shrink:0">
        ...
        <div style="position:absolute;inset:14px;border-radius:50%;background:var(--soil);display:flex;flex-direction:column;align-items:center;justify-content:center">
          <div style="font-family:'Space Grotesk',sans-serif;font-size:26px;color:var(--bone);line-height:1">${Math.round(kcal)}</div>
          <div style="font-family:'Space Grotesk',sans-serif;font-size:9px;letter-spacing:2px;color:var(--mist);text-transform:uppercase;margin-top:2px">kcal</div>
        </div>
      </div>
...
function _edLegendRow(color, label, value, unit) {
  return `<div style="display:flex;align-items:center;gap:7px">
    ...
    <span style="font-size:13px;color:var(--sand);font-family:'Space Grotesk',sans-serif">${_edFmtNum(value)}${unit}</span>
  </div>`;
}
...
function _edMacroFractionsHtml(e) {
  ...
  return `<div style="padding:14px 16px 0">
    <div style="font-family:'Space Grotesk',sans-serif;font-size:10px;letter-spacing:2px;color:var(--mist);text-transform:uppercase;margin-bottom:4px">This meal / full day target</div>
...
function _edRenderView(e) {
  ...
  const html = `
    ...
    <div style="padding:18px 16px 0">
      <div style="font-family:'Space Grotesk',sans-serif;font-size:26px;color:var(--bone);line-height:1.15">${e.hypo_correction ? '🩸 ' : ''}${e.meal || e.name || 'Unnamed entry'}</div>
      <div style="margin-top:8px;display:flex;align-items:center;gap:8px">
        ${label ? `<span style="font-family:'Space Grotesk',sans-serif;font-size:10px;letter-spacing:2px;color:${color};text-transform:uppercase;border:1px solid ${color};border-radius:20px;padding:3px 10px">${label}</span>` : ''}
...
function _edRenderEdit(e) {
  ...
  const html = `
    ...
    <div style="padding:18px 16px 0;display:flex;flex-direction:column;gap:14px">
      <div>
        <label style="display:block;font-family:'Space Grotesk',sans-serif;font-size:10px;letter-spacing:2px;color:var(--mist);text-transform:uppercase;margin-bottom:6px">Meal name</label>
        <input type="text" id="ed-f-name" class="themed-field" value="${(e.meal || e.name || 'Unnamed entry').replace(/"/g,'&quot;')}" style="${fieldStyle}">
      </div>
      <div>
        <label style="display:block;font-family:'Space Grotesk',sans-serif;font-size:10px;letter-spacing:2px;color:var(--mist);text-transform:uppercase;margin-bottom:6px">Meal type</label>
        <select id="ed-f-type" style="${fieldStyle}">${typeOpts}</select>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        <div>
          <label style="display:block;font-family:'Space Grotesk',sans-serif;font-size:10px;letter-spacing:2px;color:var(--mist);text-transform:uppercase;margin-bottom:6px">Kcal</label>
          <input type="number" id="ed-f-kcal" inputmode="decimal" value="${_edFmtNum(e.calories||0)}" style="${fieldStyle}">
        </div>
        <div>
          <label style="display:block;font-family:'Space Grotesk',sans-serif;font-size:10px;letter-spacing:2px;color:var(--mist);text-transform:uppercase;margin-bottom:6px">Protein (g)</label>
          <input type="number" id="ed-f-protein" inputmode="decimal" value="${_edFmtNum(e.protein_g||0)}" style="${fieldStyle}">
        </div>
        <div>
          <label style="display:block;font-family:'Space Grotesk',sans-serif;font-size:10px;letter-spacing:2px;color:var(--mist);text-transform:uppercase;margin-bottom:6px">Carbs (g)</label>
          <input type="number" id="ed-f-carbs" inputmode="decimal" value="${_edFmtNum(e.carbs_g||0)}" style="${fieldStyle}">
        </div>
        <div>
          <label style="display:block;font-family:'Space Grotesk',sans-serif;font-size:10px;letter-spacing:2px;color:var(--mist);text-transform:uppercase;margin-bottom:6px">Fat (g)</label>
      ...
      <div>
        <div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:6px">
          <span style="font-family:'Space Grotesk',sans-serif;font-size:10px;letter-spacing:2px;color:var(--mist);text-transform:uppercase">Portion size</span>
...
function _eddMealRowHtml(e, editing) {
  ...
  return `<div style="display:flex;justify-content:space-between;align-items:baseline;padding:10px 16px;border-bottom:1px solid var(--bark);cursor:pointer" onclick="openEntryDetail(${e.id})">
    <div>${label?`<span style="font-size:10px;color:var(--mist);font-family:'Space Grotesk',sans-serif;letter-spacing:1px;margin-right:6px">${label.toUpperCase()}</span>`:''}<span style="font-size:13px;color:var(--sand)">${name}</span></div>
...
function _eddRenderView() {
  ...
  const html = `
    ${_eddHeader(true)}
    <div style="padding:18px 16px 0">
      <div style="font-family:'Space Grotesk',sans-serif;font-size:24px;color:var(--bone);line-height:1.15">${dateLabel}</div>
      <div style="margin-top:6px"><span style="font-family:'Space Grotesk',sans-serif;font-size:10px;letter-spacing:2px;color:${color};text-transform:uppercase">${person==='gabi'?'Gabi':'Nacho'}</span></div>
    </div>
    ${_edDonutHtml(totals.kcal, totals.protein, totals.carbs, totals.fat)}
    <div style="padding:14px 16px 0">
      <div style="font-family:'Space Grotesk',sans-serif;font-size:10px;letter-spacing:2px;color:var(--mist);text-transform:uppercase;margin-bottom:4px">Day total / full day target</div>
    ...
    <div style="padding:18px 0 0">
      <div style="font-family:'Space Grotesk',sans-serif;font-size:10px;letter-spacing:2px;color:var(--mist);text-transform:uppercase;margin-bottom:4px;padding:0 16px">Meals</div>
      ${mealRows}
    </div>
    <div style="height:20px"></div>
  `;
